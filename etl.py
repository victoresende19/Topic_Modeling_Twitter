...
def tweepy_config(twitter_keys):
    ...
    recebe - twitter_keys: chaves de acesso
    retorna - api: instancia api
    """
...
def tweets_scrapy(api):
    ...
    recebe - api: instancia da API configurada
    retorna - df: dataframe com as variaveis coletadas
    """
...
def normalize(df_twt):
    ...
    recebe - df_twt: dataframe com os tweets coletados
    retorna - df_twt['processed']: coluna do dataframe que passou por normalizacao
    """
...
def lemanization(tokens):
    """'
    Lemanizacao dos dados normalizados: Reduz o token ao lema

    recebe - tokens a ser lematizada
    retorna - tokens lematizados
    """

    sentence_tokenize = [token.lemma_ for token in nlp(tokens.lower())
...
def tfidf_processing(sentences, gram):
    ...
    recebe - sentence: sentencas / gram: quantidade de palavras por tokens
    retorna - df_rankeado: dataframe com termo e rank ordenados
    """
